...
How does it work in general?  Yes, at least subjectively.  It's rather less random than the "random" plugin.  Is The Beatles Hey Bulldog similar to Jimi Hendrix Remember?  Kinda?  Of course this all will depend on the distribution of music you give for your virtual radio DJ to play.
...
embedding calculated and a vector added to the FAISS index.  This information is
kept in files that by default reside next to the Beets DB file location.
...
- Beets' own chroma / acoustic ID.  This can be used to evaluate "equality" if not "distance" so is not so good for "similarity".
- [[https://github.com/jaeheonshim/vibenet][vibenet]] gives a 7-dimensional vector in a "music emotion" semantically meaningful space (energy, speechiness, etc).  I would like to add vibenet indexing to vrdj both on its own and some way to combine it with VGGish vectors.
